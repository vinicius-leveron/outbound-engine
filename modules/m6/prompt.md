...
**OBRIGATÓRIO antes de qualquer escrita no Sheets.** Para leitura pode usar API key, mas se precisar escrever use o SHEETS_TOKEN.
...
```bash
curl -s "${SHEETS_BASE}/DM_Queue!A:H" -H "Authorization: Bearer ${SHEETS_TOKEN}"
